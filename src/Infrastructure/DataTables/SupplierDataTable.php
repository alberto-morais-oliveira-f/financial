...
class SupplierDataTable extends DataTable
{
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ->addColumn('action', function (Supplier $supplier) {
                return view('financial::suppliers.datatables.actions', ['model' => $supplier]);
            })
            ->editColumn('is_active', function (Supplier $supplier) {
                return $supplier->is_active ? 'Sim' : 'Não';
            })
            ->setRowId('uuid');
    }
    ...
    protected function getColumns(): array
    {
        return [
            ...
            Column::make('document')->title('Documento'),
            Column::make('phone')->title('Telefone'),
            Column::make('is_active')->title('Ativo')->addClass('text-center'),
